# URSA output stationary, GEMM only

> **Stable version.** This folder is frozen at the state that produced the
> dissertation results. Active development of the output stationary design
> happens in `exp/output-stationary`.

## Build and measurement flow

Five scripts, run in order. Each one feeds the next.

```
02_do_hls_all.sh -> 03_do_bitstreams.sh -> 04_do_platforms.py -> 05_do_vitis.sh -> 06_run_board.py
    IP cores          bitstreams, XSAs        platforms             ELFs          results/*.csv
```
...
- **layout** alone decides the platform, so it reaches `04`
- **test mode and cache configuration** decide the software, so they reach `05`
- **mitigation** decides the bitstream, so it reaches `03`

`06` pairs one bitstream with one ELF, chosen independently. A triplicated
...
Creates the Vitis platforms that `05` builds against, one per layout, from the
XSAs written by `03`.

**Writes:** `vitis_component/platform-ursa-vanilla-<layout>/`

One platform per layout, not per build: the platform fixes the address map and
the BSP, which do not change with array size, and a mitigated design keeps the
accelerator at the same base address as its vanilla twin. The three layouts do
need separate platforms, since each block design configures the PS
differently.

The BSP is standalone on `ps7_cortexa9_0`, with `XILTIMER_tick_timer` set to
`None`. That is the only non-default setting, and it matches the platforms
originally built by hand in the IDE. The script reads the value back from the
generated `bsp.yaml` and prints it in its final table.

```bash
source /opt/Xilinx/Vitis/2023.2/settings64.sh
vitis -s 04_do_platforms.py                          # all three layouts
LAYOUTS=bram vitis -s 04_do_platforms.py             # one layout
SA_SIZE=8 FORCE=1 vitis -s 04_do_platforms.py        # rebuild from the 8x8 XSA
```

An existing platform is skipped unless `FORCE=1`.

### Things that bite

**Run it with `vitis -s`, not `python3`.** The `vitis` module ships with the
Python bundled in Vitis; the system Python fails with
`ModuleNotFoundError: No module named 'vitis'`.

**The platforms are versioned, but they hold absolute paths.** Hundreds of
files inside each platform point at the directory where it was built. If the
repository is cloned somewhere else, regenerate them with `FORCE=1` before
running `05`.
...
Drives CMake directly, which needs the system CMake (the one bundled with
Vitis wants `libidn.so.11`) and `ESW_REPO` exported. Needs the three platforms
from `04`.
...
```bash
MODE=cnn ./05_do_vitis.sh                  # both caches on
MODE=cnn CACHE_I=off ./05_do_vitis.sh      # I-cache off
```
...
```bash
source /opt/Xilinx/Vitis/2023.2/settings64.sh
vitis -s 04_do_platforms.py
```

Three platforms, one per layout. Skip this step if they are already in
`vitis_component/` and the repository has not moved.
...
```bash
MODE=bench ./05_do_vitis.sh
MODE=bench CACHE_I=off ./05_do_vitis.sh

MODE=cnn ./05_do_vitis.sh
MODE=cnn CACHE_I=off ./05_do_vitis.sh

MODE=free ./05_do_vitis.sh
MODE=free CACHE_I=off ./05_do_vitis.sh
```
...
## 2026-09-17 - Ulisses
- Copied from `exp/output-stationary` into `fpga/os_gemm` as the stable
  version. Development continues in `exp/output-stationary`.
- Added `04_do_platforms.py`, which creates the three Vitis platforms from the
  XSAs instead of building them by hand in the IDE.
- Renumbered the flow: `04_do_vitis.sh` is now `05_do_vitis.sh`, and
  `05_run_board.py` is now `06_run_board.py`.
- Vitis platforms are now versioned in `vitis_component/`.