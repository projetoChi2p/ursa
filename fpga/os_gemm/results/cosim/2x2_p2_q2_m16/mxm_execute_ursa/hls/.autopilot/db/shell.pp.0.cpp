...
typedef struct {
   PE pe[2][2];
   data_a_t in_mtx_a_li[2];
   data_b_t in_mtx_b_tw[2];
}SA;
...
void fill_inputs_a(data_a_t *addr_a, data_a_t in_a[2], uint16_t t, uint16_t m){
    data_a_t value_a;
...
void fill_inputs_b(data_b_t *addr_b, uint16_t str_b, data_b_t in_b[2] ,uint16_t t, uint16_t m){
    data_b_t value_b;
...
void load_inputs_sa(SA *sa, data_a_t in_a[2], data_b_t in_b[2]){
#pragma HLS PIPELINE II=1

 LOAD_SA: for(uint16_t k=0;k<2;k++) {
...
#pragma HLS DATAFLOW
 sa_init(&sa);
        uint16_t call_a = a0_p/2;
        uint16_t call_b = b0_q/2;


        const uint32_t stride_a = (uint32_t)2 * m;
        const uint32_t stride_c = (uint32_t)2 * b0_q;
        ...
        TILE_ROW: for(uint16_t i=0;i<call_a;i++){
            ...
            TILE_COL: for(uint16_t j=0;j<call_b;j++){
                ...
                uint16_t t=0;
                STREAM_K: for(uint16_t k=0;k<m+2 -1+2 -1;k++){
#pragma HLS PIPELINE II=1
