...
/*********************************************/
...
#ifdef IM2COL
    // Imagem raw vai direto para BRAM_BI — URSA faz im2col internamente
    memcpy((void*)bi, (void*)in_data, ch_in * wh_in * wh_in * sizeof(pixel_t));
#else
    // ARM fez im2col — copia scratchpad para BRAM_BI
    if( (rc = populate_bi_with_scrachpad_bi(
        g_scratchpad_bi,
        m_a_cols_b_rows,
        n_c_cols_b_cols,
    ...
    aligned_sa_p(ch_out, &l_c_rows_a_rows);
        ...
        uint8_t sa_status_0 = mxm_execute_ursa_LABFT(
            ...
            addr_a, addr_b, addr_c0,
            ch_in, wh_in, wh_kernel, pad, stride,
            0
        );
    #else
        uint8_t sa_status_0 = mxm_execute_ursa_LABFT(
