...
int populate_test_image_chw_on_pixel_buffer(uint8_t out_buffer, uint8_t* pixels) {
    ...
    //for debug:
    //printf("[POPULATE_IMG] img[%d][%d][%d] - image_pixels=%d\n",IMAGE_SZ_IN,IMAGE_SZ_IN,IMAGE_CH_IN,image_pixels);
...
static int check_weigths_into_mem(weight_t* mem_check){
    ...
    //printf("[POPULATE_W] Weights check OK! Total weights: %u\n", index);
...
int populate_aw_with_all_the_weights(weight_t* aw){
    ...
    //printf("[POPULATE_W] Total weights written (with padding): %u\n", total_weights);
...
/*********** NEW WAY ACCELERATOR ************************************************************ */
    //n_c_cols_b_cols=calcular aqui

    //VERSÃO URSA COM IM2COL
    // sa_status=mxm_execute_ursa(
    //         &aw[addr_weights],     //end mtx a (pesos)
    //         l_c_rows_a_rows,       //p -> A(P)(M)
    //         &bi[0],                //end (image_after_im2col)
    //         n_c_cols_b_cols,       //q -> B(M)(Q)
    //         &ca[0],
    //         m_a_cols_b_rows,       //m -> C(P)(Q)   

    //         &in_data[0],            //end (image_before_im2col)
    //         ch_in,
    //         wh_in,
    //         wh_kernel,
    //         stride
