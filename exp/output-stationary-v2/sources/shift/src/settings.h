...
// PE arithmetic
// PE_POT (default in this IP): weights are power-of-two codes and the PE does
// shift + add instead of multiply. Memory A still holds one byte per weight,
// only the low nibble is used:
//     bit 3      sign (1 = negative)
//     bits 2..0  code: 0 -> weight 0, k -> 2^(k-1)   (1, 2, 4, ... 64)
// Code 0 MUST mean zero: the skew regions of fill_inputs_a and the padded
// rows inject 0, and they have to contribute nothing.
// Build with -DPE_MUL to get the original multiplier PE back.
#if !defined(PE_MUL) && !defined(PE_POT)
    #define PE_POT
#endif
...
// Math SA
    ...
    typedef ap_int  <8>  mem_a_t;  // A as stored in memory (one byte per weight)
  #ifdef PE_POT
    typedef ap_uint <4>  data_a_t; // PoT code travelling through the array
  #else
...
#else
    typedef int8_t  mem_a_t;
  #ifdef PE_POT
    typedef uint8_t data_a_t;      // low nibble used
  #else
