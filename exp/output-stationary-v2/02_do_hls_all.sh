...
# Target clock period for HLS, in ns. 10 -> 100 MHz, 5 -> 200 MHz. This is the
# period C synthesis schedules against; the PL clock itself is set in the block
# design, so the two have to agree.
CLK_NS=10
...
for acc in ${ACC_BITS_LIST[*]}; do
    ...
    sed -i "s#__MAXBW__#${maxbw}#g"             ${CFG}
    sed -i "s#__CLK__#${CLK_NS}#g"              ${CFG}
