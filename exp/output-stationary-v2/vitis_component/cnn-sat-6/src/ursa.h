...
/* ─── PE arithmetic ──────────────────────────────────────────────────────────
   Must match the bitstream. The shift IP (mxm_execute_ursa 3.0) reads a 4-bit
   power-of-two code per weight from A, so the ARM has to round the weights to
   2^k and encode them before staging. Loading int8 weights into the shift IP
   does not hang, it just computes garbage (measured: 1/6).

   Default here is the shift IP. For the multiplier bitstream (1.0), pass
   -DPE_MUL or change the define below.                                      */
#if !defined(PE_POT) && !defined(PE_MUL)
    #define PE_MUL
#endif

#if defined(PE_POT) && defined(PE_MUL)
    #error "Select only one PE: PE_POT (shift bitstream) or PE_MUL (multiplier bitstream)."
#endif
