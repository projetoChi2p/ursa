...
/* Zeroes the three stage channels and the per-layer table. Call once before
   the timed pass. */
void conv_engine_profile_reset(void);

/* Per-layer split of the timed pass: im2col, transfer, gemm and the whole
   convolution call, for each of the CONV_PROFILE_LAYERS layers. */
#define CONV_PROFILE_LAYERS 3
...
#ifdef PE_POT
/* Rounds the int8 weights to powers of two in place (and adjusts the layer
   scales). Call once, before populate_aw_with_all_the_weights. */
