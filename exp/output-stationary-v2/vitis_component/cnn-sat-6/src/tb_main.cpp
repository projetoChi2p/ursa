...
/* Network name and app version in the banner. v1.0 is the trained T3;
   v2.0 is URSA-FIT, the synthetic SA-native topology (CNN_NETWORK_SYN). */
#if defined(CNN_NETWORK_SYN)
    #define NET_NAME    "URSA-FIT"
    #define APP_VER     "v2.0"
#else
    #define NET_NAME    "T3"
    #define APP_VER     "v1.0"
                                 ...
                                 uint8_t  *predicted_out)
{
    ...
    /* Synthetic weights are generated before anything consumes them, for
       both engines. Outside the timed pass: inference_count 1 is the
       accuracy/warm-up pass. */
...
#ifdef PE_POT
    /* Shift-PE build: round the weights to powers of two once, for both
       engines, so the ARM reference computes exactly what URSA computes. */
    if (inference_count == 1) {
        pot_prepare_weights();
    }
#endif
...
#ifdef CNN_NETWORK_SYN
    (void)i; (void)max_out;
    predicted_label = syn_head_classify(g_pooling_buffer);
#else
...
/* Forces FCLK0 at run time, for when the platform's ps7_init is stale.
   IO PLL at 1000 MHz: div0 = 8 -> 125 MHz, 10 -> 100 MHz, 7 -> 142.86 MHz.
...
static void ursa_smoke_test(XMxm_execute_ursa *px)
{
    UINTPTR ap = px->Ap_BaseAddress;
    volatile int8_t   *a = (volatile int8_t   *)BRAM_AW_BASEADDR;
    volatile uint8_t  *b = (volatile uint8_t  *)BRAM_BI_BASEADDR;
    volatile uint32_t *c = (volatile uint32_t *)BRAM_CA_BASEADDR;

    /* A = PoT code 1 (weight +1), B = 1, so every C entry must be SMOKE_M */
    for (uint32_t i = 0; i < SA_SIZE * SMOKE_M; i++) a[i] = 1;
    for (uint32_t i = 0; i < SMOKE_M * SA_SIZE; i++) b[i] = 1;
    for (uint32_t i = 0; i < SA_SIZE * SA_SIZE; i++) c[i] = SENTINEL;
    Xil_DCacheFlush();

    XMxm_execute_ursa_Set_a0_p(px, SA_SIZE);
    XMxm_execute_ursa_Set_b0_q(px, SA_SIZE);
    XMxm_execute_ursa_Set_m(px, SMOKE_M);
    XMxm_execute_ursa_Set_addr_a0(px, BRAM_AW_BASEADDR);
    XMxm_execute_ursa_Set_addr_b0(px, BRAM_BI_BASEADDR);
    XMxm_execute_ursa_Set_addr_c0(px, BRAM_CA_BASEADDR);

    XMxm_execute_ursa_Start(px);
    usleep(10000);                                   /* 10 ms: far more than enough */
    uint32_t ctrl = Xil_In32(ap);

    Xil_DCacheInvalidate();
    uint32_t written = 0, correct = 0, first_untouched = 0xFFFF;
    for (uint32_t i = 0; i < SA_SIZE * SA_SIZE; i++) {
        if (c[i] != SENTINEL) { written++; if (c[i] == SMOKE_M) correct++; }
        else if (first_untouched == 0xFFFF) first_untouched = i;
    }
    xil_printf("[smoke] ap_ctrl=0x%08lx  C written %lu/%u  correct %lu  first untouched %lu\n\r",
               (unsigned long)ctrl, (unsigned long)written, SA_SIZE * SA_SIZE,
               (unsigned long)correct, (unsigned long)first_untouched);
    xil_printf("[smoke] c[0]=0x%08lx c[1]=0x%08lx c[%u]=0x%08lx\n\r",
               (unsigned long)c[0], (unsigned long)c[1],
               SA_SIZE * SA_SIZE - 1, (unsigned long)c[SA_SIZE * SA_SIZE - 1]);
}
...
int main(void)
{
    ...
    /* no print */
  #else
    fprintf(stderr, "\n### CNN SAT-6 " NET_NAME " " ENGINE_NAME " " LAYOUT_NAME " [" APP_VER "] ###\n");
    printf("SA_SIZE=%d, acc=%d bits\n", SA_SIZE, ACC_BITS);
  #ifdef CNN_NETWORK_SYN
    printf("net=URSA-FIT (random weights, TIMING ONLY) C1=%d C2=%d C3=%d aliasA=%d\n",
           CONV1_CH_OUT, CONV2_CH_OUT, CONV3_CH_OUT, SYN_WEIGHTS_ALIASED);
    printf("GEMM P,M,Q: [%d,%d,256] [%d,%d,64] [%d,%d,64]  padded P: %d %d %d\n",
           CONV1_ROW, CONV1_COL, CONV2_ROW, CONV2_COL, CONV3_ROW, CONV3_COL,
           CONV1_PADDED_ROW, CONV2_PADDED_ROW, CONV3_PADDED_ROW);
  #else
    printf("net=T3 (trained weights)\n");
  #endif
  #ifdef PE_POT
    printf("pe=POT (shift-add, weights rounded to 2^k, no retraining)\n");
  #else
    printf("pe=MUL\n");
  #endif
    printf("cache I=%s\n", CACHE_EN_I ? "on" : "off");
         ...
         fps_x100 = 1e8 * INFER_ITERS / us_total                              */
    {
           ...
           here comes from the same pass, the subtraction is exact.         */
        {
            ...
            conv_engine_print_layer_profile((uint32_t)n_total);   /* por camada */
        }
    }
  #else
    fprintf(stderr, "------------------------------------------------------------\n");
...
terminate_failed:
    fprintf(stderr, "### SAT-6 " NET_NAME " failed ###\n");
    return EXIT_FAILURE;
}