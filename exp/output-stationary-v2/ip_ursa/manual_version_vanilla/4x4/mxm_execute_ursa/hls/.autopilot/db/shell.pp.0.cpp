...
static void fill_inputs_a(const a_word_t a_buf[4][(256 / 4)],
                          data_a_t in_a[4], uint16_t t, uint16_t m)
{
#pragma HLS INLINE
...
#pragma HLS UNROLL
 ...
 if (t >= i && t < i + m) {
            ...
            const uint16_t wi = k / 4;
            const uint16_t by = k % 4;
...
static void fill_inputs_b(const data_b_t b_buf[4][256],
                          data_b_t in_b[4], uint16_t t, uint16_t m)
{
#pragma HLS INLINE
...
static void load_inputs_sa(SA *sa, data_a_t in_a[4], data_b_t in_b[4])
{
#pragma HLS INLINE
...
static void load_tile_a(const a_word_t *a, uint32_t word_off, uint16_t wpr,
                        a_word_t a_buf[4][(256 / 4)])
{
    uint16_t r = 0, c = 0;
    const uint32_t n_words = (uint32_t)4 * wpr;
...
#pragma HLS LOOP_TRIPCOUNT min=36 max=144 avg=144
 b_word_t w = b[idx];
        VITIS_LOOP_127_1: for (uint16_t jj = 0; jj < 4; jj++) {
#pragma HLS UNROLL
...
#pragma HLS INTERFACE mode=m_axi port=addr_b0 bundle=bi offset=slave num_read_outstanding=8 num_write_outstanding=8 max_read_burst_length=64 max_write_burst_length=16 depth=1024
...
#pragma HLS ARRAY_PARTITION variable=sa complete dim=1
 ...
 sa_init(&sa);
    ...
    const uint16_t call_a = a0_p / 4;
    const uint16_t call_b = b0_q / 4;

    const uint16_t wpr = (m + 4 - 1) / 4;
    const uint32_t stride_a = (uint32_t)4 * wpr;
    const uint32_t stride_c = (uint32_t)4 * b0_q;
    const uint32_t qw = b0_q / 4;
...
#pragma HLS LOOP_TRIPCOUNT min=16 max=64 avg=16
 ...
 load_tile_b(addr_b0, j, qw, m, b_buf);
            ...
            STREAM_K: for (uint16_t t = 0; t < m + 2*(4 -1); t++) {
#pragma HLS PIPELINE II=1
