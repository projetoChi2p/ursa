...
/* Campaign-equivalent software condition: both caches off for the whole run,
 * matching the CAMPAIGN build used during irradiation. */
#ifndef CACHE_EN_I
    #define CACHE_EN_I 0
#endif
...
/* ─── ARM baseline buffers ───────────────────────────────────────────────────
   The software baseline drives no accelerator, so nothing requires its data
   at a fixed address. Ordinary arrays, like g_scratchpad_* in the campaign
   build. Declared for both host and board.                                  */
#ifndef ENGINE_ARM
    #define ENGINE_ARM 0
#endif
