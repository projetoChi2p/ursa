...
{
    /* APP_TIMER_COUNTS_PER_MICROSECOND is an integer divide of the counter
       frequency by a million, so on a 666.67 MHz core it evaluates to 333
       where the true rate is 333.33. Dividing by it made every reported
       interval 0.1% long, and every derived frame rate 0.1% low: a small
       error, but a systematic one present in every number.

       Scaling first and dividing by the full rate is exact, and integer
       throughout. elapsed is 32 bits, so the product needs 64. */
    return (u32)(((uint64_t)app_times[timer].elapsed * 1000000u)
                 / (uint64_t)APP_TIMER_COUNTS_PER_SECOND);
}
