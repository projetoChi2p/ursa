...
# 31 "/home/ulisses/Projects/ursa/exp/output-stationary/vitis_component/inp_data/hls_src/core/../settings.h"
...
# 32 "/home/ulisses/Projects/ursa/exp/output-stationary/vitis_component/inp_data/hls_src/core/../settings.h" 2
...
void fill_inputs_a(data_a_t *addr_a, data_a_t in_a[8], uint16_t t, uint16_t m){
    data_a_t value_a;
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_14_1: for(uint16_t i=0;i<8;i++){
#pragma HLS UNROLL
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_42_1: for(uint16_t j=0;j<8;j++){
#pragma HLS UNROLL
...
#pragma HLS PIPELINE II=1
 ...
 VITIS_LOOP_68_1: for(uint16_t k=0;k<8;k++) {
...
#pragma HLS INTERFACE mode=m_axi port=casted_c0 bundle=ca offset=slave num_read_outstanding=8 num_write_outstanding=8 max_read_burst_length=64 max_write_burst_length=16 depth=200
...
#pragma HLS DATAFLOW
 sa_init(&sa);
        ...
        VITIS_LOOP_131_1: for(uint16_t i=0;i<call_a;i++){
            VITIS_LOOP_132_2: for(uint16_t j=0;j<call_b;j++){
                addr_sa_a = (data_a_t*)(casted_a0 + i * 8 * m);
                addr_sa_b = (data_b_t*)(casted_b0 + j * 8);


                uint16_t t=0;
                VITIS_LOOP_138_3: for(uint16_t k=0;k<m+8 -1+8 -1;k++){
#pragma HLS PIPELINE II=1
