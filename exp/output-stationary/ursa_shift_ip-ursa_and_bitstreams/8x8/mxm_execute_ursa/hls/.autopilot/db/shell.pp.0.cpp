...
typedef struct {
   PE pe[8][8];
   data_a_t in_mtx_a_li[8];
   data_b_t in_mtx_b_tw[8];
}SA;
...
void fill_inputs_a(mem_a_t *addr_a, data_a_t in_a[8], uint16_t t, uint16_t m){
    data_a_t value_a;
...
void fill_inputs_b(data_b_t *addr_b, uint16_t str_b, data_b_t in_b[8] ,uint16_t t, uint16_t m){
    data_b_t value_b;
...
void load_inputs_sa(SA *sa, data_a_t in_a[8], data_b_t in_b[8]){
#pragma HLS PIPELINE II=1

 LOAD_SA: for(uint16_t k=0;k<8;k++) {
...
#pragma HLS INTERFACE mode=m_axi port=casted_a0 bundle=aw offset=slave num_read_outstanding=8 num_write_outstanding=8 max_read_burst_length=64 max_write_burst_length=16 depth=4096



#pragma HLS INTERFACE mode=m_axi port=casted_b0 bundle=bi offset=slave num_read_outstanding=8 num_write_outstanding=8 max_read_burst_length=64 max_write_burst_length=16 depth=4096



#pragma HLS INTERFACE mode=m_axi port=casted_c0 bundle=ca offset=slave num_read_outstanding=8 num_write_outstanding=8 max_read_burst_length=64 max_write_burst_length=16 depth=4096
...
#pragma HLS ARRAY_PARTITION variable=sa complete dim=1

 {
        ...
        sa_init(&sa);
        uint16_t call_a = a0_p/8;
        uint16_t call_b = b0_q/8;


        const uint32_t stride_a = (uint32_t)8 * m;
        const uint32_t stride_c = (uint32_t)8 * b0_q;
        ...
        TILE_ROW: for(uint16_t i=0;i<call_a;i++){
            ...
            TILE_COL: for(uint16_t j=0;j<call_b;j++){
                ...
                uint16_t t=0;
                STREAM_K: for(uint16_t k=0;k<m+8 -1+8 -1;k++){
#pragma HLS PIPELINE II=1
...
# 234 "/home/ulisses/Projects/ursa/exp/output-stationary/vitis_component/inp_data_exp/hls_src/core/shell.cpp"
