...
#pragma HLS PIPELINE II=1
 ...
 INIT_ROW: for(i=0;i<8;i++){
#pragma HLS UNROLL
 INIT_COL: for(j=0;j<8;j++){
#pragma HLS UNROLL
 pe_init(&sa->pe[i][j]);
      }
   }
...
#pragma HLS PIPELINE II=1
 ...
 RESET_ROW: for(uint16_t i=0;i<8;i++){
#pragma HLS UNROLL
 RESET_COL: for(uint16_t j=0;j<8;j++){
#pragma HLS UNROLL
 pe_reset(&sa->pe[i][j]);
        }
    }
...
#pragma HLS UNROLL
 sa->pe[i][0].li = sa->in_mtx_a_li[i];
   ...
   LINK_H_COL: for(j=1;j<8;j++) {
#pragma HLS UNROLL
 LINK_H_ROW: for(i=0;i<8;i++) {
#pragma HLS UNROLL
 sa->pe[i][j].li = sa->pe[i][j-1].ri;
   ...
   LINK_V_ROW: for(i=1;i<8;i++) {
#pragma HLS UNROLL
 LINK_V_COL: for(j=0;j<8;j++) {
#pragma HLS UNROLL
 sa->pe[i][j].tw = sa->pe[i-1][j].bw;
   ...
   PE_ROW: for(i=0;i<8;i++) {
#pragma HLS UNROLL
 PE_COL: for(j=0;j<8;j++) {
#pragma HLS UNROLL
...
STORE_ROW: for(uint16_t i=0;i<8;i++){
#pragma HLS UNROLL
STORE_COL: for(uint16_t j=0;j<8;j++){
#pragma HLS UNROLL
